...
class TestCurl(unittest.TestCase):
    def test_x_form_urlencoded_explicit(self):
        ...
        expected = JobTemplate(
            endpoint="http://localhost:3000/blahblah",
            children=[],
            method="POST",
            dataType="blahblah",
    ...
    def test_json_query_params_fail(self):
        ...
        self.assertEqual(
            str(context.exception), "Query parameters are not supported for POST requests with JSON content type"
        )
  ...
  'https://connection.keboola.com/v2/storage/events?sinceId={sinceId}&maxId={maxId}&component={component}&configurationId={configurationId}&runId={runId}&q={q}&limit={limit}&offset={offset}'"""
        expected = JobTemplate(
            endpoint="https://connection.keboola.com/v2/storage/events",
            children=[],
            method="GET",
            dataType="events",
            dataField={"path": ".", "separator": "."},
            params={
                ...
                "configurationId": "_configurationId_",
                "runId": "_runId_",
                "q": "_q_",
                "limit": "_limit_",
                "offset": "_offset_",
    ...
    def test_url_w_placeholders(self):
        ...
        expected = JobTemplate(
            endpoint="https://connection.{{stack}}.keboola.com/v2/storage/events",
            children=[],
            method="GET",
            dataType="events",
            dataField={"path": ".", "separator": "."},
